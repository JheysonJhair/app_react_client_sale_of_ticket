...
export function SidebarItem({ name, url, icon }: Props) {
  ...
  return (
    ...
    >
      <div
        className={`${
          isActive && "bg-popover text-black"
        } p-2 w-full group-hover:bg-popover  group-hover:text-white rounded transition-all duration-75 origin-left mt-5 flex gap-5 ${
          isActive  && "bg-popover origin-left text-white"
        }`}
      >
        <span>{icon}</span>
